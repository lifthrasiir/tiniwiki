<h1 class="heading"><a href="<?=$wikiPath?>/<?=$PAGE?>?links2" class="_noline">이 페이지를 링크하는 페이지들</a></h1>
<hr class="hr" />
<h1 class="heading">이 페이지가 링크한 페이지들</h1>
<?php
$_pagelink = count($pagelink);
if(!page_exists($page)) {
	echo "<h2 class=\"heading\">이 페이지는 존재하지 않습니다.</h2>\n";
} elseif($_pagelink > 0) {
	ksort($pagelink);
